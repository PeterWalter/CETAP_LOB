...
namespace CETAP_LOB.Model
{
  public class CompositeService : ICompositeService
  {
    ...
    private Dir_categories myDir;
        ...
        public Task<List<CompositBDO>> GetAllNBTScoresAsync(int page, int size)
        {
            throw new NotImplementedException();
        }
    ...
    private void CompositToLogisticsComposite()
    {
      ...
      foreach (var data in this.AllScores.AsParallel<CompositBDO>().Select(mydata => new
      {
        ...
        Mat_Lang = mydata.MatLanguage,
        mm = mydata.Batch,
        TestType = this.MytestType(mydata.Batch),
        province = this.getProvince(mydata.VenueCode)
      }))
    ...
    //{
    //  return this.getProvinceByID(this.AllVenues.Where<VenueBDO>((Func<VenueBDO, bool>) (x => x.VenueCode == venuecode))
    //             .FirstOrDefault<VenueBDO>().ProvinceID).Name;
    //}
    ...
    //      CompositeService.CompositBDOToCompositDAL(results, composit);
